 ...
 *
 *   TODO (maybe):
 *     - Interrupt calls: call frome one to the other to reduce code lines.
 *     - Move color definitions to enum?
 ...
 *         -> Probaly yes (.obj file gets created), but it probably doesn't get linked if there are no calls to the
 *            method UNLESS it is a static method (?)
 *         -> Perhaps use an IFDEF around the whole dbprint.c file.
 *
 ...
 *     By using the volatile keyword, you force the compiler to not optimize these
 *     variables. If it would not be used some strange behavior could arise where
 *     the compiler doesn't notice something is called, because it can perhaps only be
 *     called once it's on the device.
 *
 *   Extern
 ...
 *         Static variables or functions are constantly kept in the memory. By using
 *         the keyword you force the compiler to not subject these variables
 *         or functions to optimizations.
 *
 *
 * ******************************************************************************
 *
